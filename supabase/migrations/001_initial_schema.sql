...
create table accounts (
  ...
  groq_api_key text,
  groq_model text,
  groq_temperature numeric(3,2) not null default 0.85 check (groq_temperature >= 0 and groq_temperature <= 2),
  groq_max_tokens integer not null default 180 check (groq_max_tokens between 32 and 1000),
  buffer_access_token text,
  buffer_profiles text[] not null default '{}',
  buffer_channel_ids text[] not null default '{}',
  platforms publish_platform[] not null default '{x}',
  schedule_interval_minutes integer not null default 15 check (schedule_interval_minutes between 5 and 1440),
  schedule_time_slots text[] not null default '{}',
