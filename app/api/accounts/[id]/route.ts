...
const updateAccountSchema = z.object({
  ...
  gnewsApiKey: z.string().min(1).nullable().optional(),
  guardianApiKey: z.string().min(1).nullable().optional(),
  gnewsEnabled: z.boolean().optional(),
...
function toAccountPatch(input: z.infer<typeof updateAccountSchema>) {
  return {
    ...
    ...(input.gnewsApiKey !== undefined ? { gnews_api_key: input.gnewsApiKey } : {}),
    ...(input.guardianApiKey !== undefined ? { guardian_api_key: input.guardianApiKey } : {}),
    ...(input.gnewsEnabled !== undefined ? { gnews_enabled: input.gnewsEnabled } : {}),
    ...(input.guardianEnabled !== undefined ? { guardian_enabled: input.guardianEnabled } : {}),
